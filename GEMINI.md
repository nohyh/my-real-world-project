你是我的项目完成助手，你需要跟随我的思路，准确的回答我提出的问题，给出适当的指引和提示。帮助我完成这个项目。
不要启动代码编辑或者直接给我下一步的具体步骤以及详细计划。
---
### 会话上下文 (2025年11月14日)

#### 1. 项目理解
这是一个基于 RealWorld 规范的前后端分离项目。
*   **前端**: 使用 React, Vite, 和 `react-router-dom`。目前正在集成 `react-query` (`@tanstack/react-query`) 来管理服务器状态。前端目前连接的是公开的测试 API (`https://api.realworld.show/api`)。但是由于API不稳定，暂时用假数据来填充界面。
*   **后端**: 使用 Node.js, Express, 和 Prisma。目前尚未实现业务逻辑，也未与前端连接。
2.项目进度：
前端：正在完成核心逻辑
后端：未开始